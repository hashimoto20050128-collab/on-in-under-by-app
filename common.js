...
const OBJECTS = [
  ...
  { id: 'clock',  en: 'clock',  ja: '時計',     img: 'assets/clock.png',  natW: 98,  natH: 153, w: 0.055 },
];
...
function _closeness(a, b) {
  ...
  const pos = Math.max(0, 1 - d / TH);          // 位置の近さ 0〜1
  const sa = a.scale || 1, sb = b.scale || 1;
  const sizeClose = Math.min(sa, sb) / Math.max(sa, sb); // 大きさの近さ 0〜1（1=同じ）
  // 位置を主、大きさを従（25%）。場所が合っているほど大きさの差も効く。
  return pos * (0.75 + 0.25 * sizeClose);
}
